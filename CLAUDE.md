...
**`readings`**: id, question_type, question, cards (TEXT JSON), spread_type, reading_result, reading_source (`template`|`ai`), user_id (FK), is_public (SMALLINT 1/0), created_at, deleted_at (soft delete), hidden_at. Indexes on user_id, created_at, reading_source. No FK ON DELETE CASCADE — hard-deleting a user manually deletes their tokens + readings + feedback first (see admin route); `chat_conversations` rows cascade off readings and `chat_usage` rows off users.

**`chat_conversations`**: id, user_id (FK, `ON DELETE CASCADE`), reading_id (FK, `ON DELETE CASCADE`), created_at, updated_at. `UNIQUE(user_id, reading_id)` — exactly one conversation per reading. A conversation is wiped when the user leaves the chat (exit-to-burn), so rows are transient. Both FKs cascade so admin hard-deleting a reading/user doesn't trip an FK violation.
...
**`chat_usage`**: user_id (FK, `ON DELETE CASCADE`), week_start (DATE), sent_count (INT), `PRIMARY KEY (user_id, week_start)`. Weekly spirit-chat quota counter, decoupled from `chat_messages` so burning a conversation (CASCADE delete) can't refund already-used quota. Buckets are Monday-based in UTC; stale buckets (>8 weeks) are deleted at startup.
...
- **Auth middleware**: `authMiddleware` verifies Bearer token into `req.user`. `requireRole(...roles)` wraps auth and **always reads the user's current role from the DB** (the JWT role claim can be up to 15 minutes stale — a demoted admin must lose access immediately); a deleted user is rejected with 401.
- **Registration**: `POST /api/auth/register` requires `acceptedTerms === true` and enforces password strength (≥8 chars, ≥1 uppercase, ≥1 digit) server-side. Stores `accepted_terms_version`/`accepted_terms_at` on the user. The first-100 promo slot counter and the user insert run in **one transaction** so a failed registration (e.g. a concurrent duplicate-email race) can't burn a promo slot. The `users` table columns are added via `ALTER TABLE ... IF NOT EXISTS` at startup (same pattern as the feedback columns).
- **Feedback**: `POST /api/feedback` (auth) is rate-limited per **user** (10/15min) with an `ipKeyGenerator` fallback — express-rate-limit v8 requires the helper when a `keyGenerator` touches `req.ip`, otherwise it raises `ERR_ERL_KEY_GEN_IPV6`. Associating a reading validates ownership + `deleted_at IS NULL`. `GET /api/feedback/unread` returns replies with `reply IS NOT NULL AND user_seen_at IS NULL`; `POST /api/feedback/read-all` marks them seen. Reverting a feedback to `open` in the admin route clears `reply`/`replied_at` so old replies can't re-surface as unread.
- **Readings creation**: `POST /api/readings` requires auth. Draws via Fisher-Yates on the 78 cards, stores only the drawn metadata (cardId/position/spreadPosition) as JSON in `cards`, and always uses `templateReading()` at creation. The AI upgrade endpoint reconstructs full card objects from that JSON before calling `aiReading()`. `question` is trimmed and capped at 500 chars (it is persisted and re-sent to the AI on every upgrade/chat call). Batch endpoints (`batch-delete`/`batch-hide`/`batch-unhide`, admin batch-delete) sanitize ids to unique positive integers, max 500 — garbage ids would otherwise 500 inside the array SQL param.
...
- **AI reading**: `aiReading()` builds a Chinese prompt, calls `{OPENCODE_BASE_URL}/chat/completions` (30s timeout), strips markdown italics/lists but keeps `### ` headers, and returns `null` on any failure (route responds 502). Settings are read DB-first with env fallback, except the API key. `AI_MAX_TOKENS` is capped at 8000 for `aiReading` (2000 for `aiChat`) so a misconfigured admin value can't balloon the bill. `POST /api/readings/:id/ai-reading` is rate-limited per user (10/15min) because failed AI calls don't consume quota — without the cap, retries could burn unlimited API credits.
- **Profile**: `PUT /api/profile` strips HTML tags from `display_name` and caps it at 50 chars; `avatar_url` must be http(s) and is capped at 500 chars. Empty display name keeps the existing one (COALESCE to `NULL`).
- **Spirit chat** (`/api/chat`, auth-only): one conversation per reading (`UNIQUE(user_id, reading_id)`, idempotent create/get). `aiChat()` builds a system prompt (the whole spread forms the spirit's voice; anti prompt-injection, no absolute predictions, safety caveats for medical/legal/finance/self-harm) plus reading context (question + spread + initial reading) plus the last 20 messages plus the user's message. Send saves the user message + assistant reply in one transaction only after the AI returns; the send route reconstructs full card objects via `drawnCard.cardId ?? drawnCard.id` to support both server-drawn readings (`cardId`) and offline-synced ones (full card objects keyed by `id`). Every endpoint checks conversation ownership via `user_id`; messages have no own user column — ownership flows through the conversation. Leaving a non-empty chat runs the "exit-to-burn" flow: a confirm modal then `DELETE` wipes the whole conversation (refresh still restores — only intentional leave deletes). Browser back is intercepted by a history guard and routes into the same confirm flow; a failed burn keeps the modal open with a "仍要离开" escape rather than silently skipping the wipe or trapping the user. Quota: free 10/week, premium 100/week user messages, admin unlimited, counted via the `chat_usage` table (Monday-week UTC buckets; a slot is atomically reserved before the AI call and refunded on AI/save failure — burning a conversation never refunds used quota); a per-user `express-rate-limit` (10/15min) guards the send endpoint. `POST /api/chat/messages/:id/feedback` sets/clears `user_rating` (up/down).
...
client/src/
  ...
  components/                    NavBar (dropdown), BackButton, PageContainer, FeedbackReplyModal, LegalModal, ErrorBoundary (render-crash fallback)
...
`api.ts`'s `apiFetch()` auto-attaches the Bearer header, and on 401 acquires a mutex (`_refreshPromise`) to prevent concurrent refresh storms, retries once, then calls `_onAuthExpired`. If the retried request **still** 401s, the session is treated as dead and `_onAuthExpired` fires too (no half-authenticated state). `createReading()` falls back to localStorage **only on network errors** — a server rejection (e.g. quota exceeded) surfaces the error instead of silently creating a local reading. `fetchReadings()` merges server + local readings and sorts by `createdAt` descending so offline readings interleave correctly.
...
Admin settings page edits OPENCODE_BASE_URL, AI_MODEL, AI_MAX_TOKENS in the DB. OPENCODE_API_KEY is configured only via `server/.env` (or Render env vars). When the AI call runs, base URL/model/max tokens resolve DB-first (falling back to env), while the API key comes strictly from env. `PUT /api/admin/settings` validates AI_MAX_TOKENS as a digits-only positive integer, and `aiReading`/`aiChat` both parse it through `resolveMaxTokens()` (non-integer/≤0 falls back to 4000) so a bad value can't become `NaN`/`null` in the API request.
...
8. ReadingResultPage parses `### ` sections from readingResult, polls `/api/readings/:id` every 2s if result is still empty, and gives up after 15 attempts (30s) showing a failure state instead of polling forever
